...
class RequestManager:
    ...
    def _fetch_with_scrapling(self, url: str, crawl_delay=None) -> Optional[Tuple[str, int]]:
        """Use Scrapling Fetcher for anti-bot resistant static requests."""
        try:
            ...
            if status == 200:
                ...
                if "Cloudflare" in content and "Attention Required" in content:
            ...
            if "certificate" in err.lower() or "ssl" in err.lower():
                try:
                    log(f"Scrapling TLS issue for {url}; retrying with verify=False")
                    response = Fetcher.get(
                        url,
                        timeout=45,
                        retries=1,
                    ...
                    log(f"Scrapling fallback error for {url}: {nested}")
            else:
                log(f"Scrapling error for {url}: {e}")
            return None, 0
...
def discover_product_urls(scraper, crawl_delay=None, robots_sitemap=None):
    ...
    for sitemap_url in sitemap_urls:
        ...
        if content:
            ...
            if any('sitemap' in url.lower() for url in urls[:10]):
                log(f"Found sitemap index with {len(urls)} child sitemaps")
                # Process each child sitemap
                for child_sitemap in urls[:13]:  # Limit to first 10 to avoid overwhelming
...
def main():
    log("=" * 60)
    log("CYMAX.COM PRODUCT SCRAPER (Scrapling Engine)")
    log("=" * 60)
