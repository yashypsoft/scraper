...
Python port of the PHP Validate class with NVIDIA LLM integration
"""
...
NVIDIA_API_KEY = "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"
NVIDIA_BASE_URL = "https://integrate.api.nvidia.com/v1/chat/completions"

MODEL_EXTRACTION = "google/gemma-3-1b-it"
MODEL_MATCHING = "meta/llama-3.2-1b-instruct"
...
class ImageMatchResult:
    """Result from vision model matching."""
    is_match: bool = False
    confidence: ConfidenceLevel = ConfidenceLevel.LOW
    similarity_score: float = 0.0
    reason: str = ""
...
class NVIDIALLMClient:
    """Client for NVIDIA API with all three models."""
    ...
    def _call_model(self, model: str, prompt: str, system_prompt: str = "", 
                    temperature: float = 0.2, max_tokens: int = 500) -> str:
        """Make API call to NVIDIA endpoint."""
        cache_key = f"{model}:{hash(prompt + system_prompt)}"
        ...
        try:
            ...
            print(f"MODEL: {model}")
            print("PROMPT:")
            print(prompt[:500])  # truncate to avoid huge logs
            print("\nRESPONSE:")
            print(result)
            print("=============================================\n")
            ...
            print(f"[NVIDIA LLM] Error calling {model}: {e}")
            return ""
    
    def extract_from_url(self, url: str) -> LLMExtractionResult:
        """Extract product identifiers from URL using Gemma 3."""
        prompt = f"""
Extract product information from this URL. The URL may contain product identifiers like MPN, SKU, or GTIN.

URL: {url}

Extract:
1. MPN (manufacturer part number) - look for alphanumeric codes like 30246, VN-CH-1208W-OSA-RTA
2. SKU - similar to MPN, often found in URL
3. GTIN (12-14 digit number) - look for numbers like 0840412302466
4. Product name from URL slug (the last part of the URL path)
5. Brand name if present in URL
...
"""
        response = self._call_model(MODEL_EXTRACTION, prompt, 
                                   "You extract product identifiers from URLs. Return only valid JSON.")
        
        result = LLMExtractionResult(raw_response=response)
        try:
            # Extract JSON from response
            json_match = re.search(r'\{[^{}]*\}', response, re.DOTALL)
            if json_match:
                data = json.loads(json_match.group())
                result.mpn = data.get("mpn", "").strip()
                result.sku = data.get("sku", "").strip()
                result.gtin = data.get("gtin", "").strip()
                result.product_name = data.get("product_name", "").strip()
                result.brand = data.get("brand", "").strip()
                if result.mpn or result.sku or result.gtin:
                    result.confidence = ConfidenceLevel.MEDIUM
        except (json.JSONDecodeError, KeyError) as e:
            print(f"[LLM] Failed to parse extraction: {e}")
        return result
    
    def match_products(self, system_product: Dict, competitor_product: Dict) -> LLMMatchResult:
        """Match system product with competitor product using Llama 3.2."""
        prompt = f"""
Determine if these two product records represent the SAME product.

IMPORTANT RULES:
- MPN/SKU match is the STRONGEST signal (highest priority)
- Ignore GTIN mismatch if MPN or SKU matches
- Product names may vary (marketing vs structured)
- Brand must match
- Minor wording differences are OK

DECISION LOGIC:
1. If MPN/SKU matches → VERY LIKELY SAME
2. If name + brand + structure match → SAME
3. GTIN missing or mismatch → IGNORE unless everything else mismatches

SYSTEM PRODUCT:
- Name: {system_product.get('product_name', '')}
- SKU: {system_product.get('sku', '')}
- MPN: {system_product.get('mpn', '')}
- GTIN: {system_product.get('gtin', '')}
- Brand: {system_product.get('brand_label', '')}
- Category: {system_product.get('cat', '')}

COMPETITOR PRODUCT:
- Name: {competitor_product.get('Ref Product Name', '')}
- SKU/MPN: {competitor_product.get('Ref MPN', '')}
- GTIN: {competitor_product.get('Ref GTIN', '')}
- Brand: {competitor_product.get('Ref Brand Name', '')}
- URL: {competitor_product.get('Ref Product URL', '')}
...
"""
        response = self._call_model(MODEL_MATCHING, prompt,
                                   "You are a product matching expert. Return only valid JSON.")
        
        result = LLMMatchResult(raw_response=response)
        try:
            json_match = re.search(r'\{[^{}]*\}', response, re.DOTALL)
            if json_match:
                data = json.loads(json_match.group())
                ...
                result.confidence = ConfidenceLevel[conf_str] if conf_str in ConfidenceLevel.__members__ else ConfidenceLevel.LOW
                result.reason = data.get("reason", "")
                result.similarity_score = float(data.get("similarity_score", 0.0))
        except (json.JSONDecodeError, KeyError, ValueError) as e:
            print(f"[LLM] Failed to parse match: {e}")
        return result
    
    def compare_images(self, image_url1: str, image_url2: str) -> ImageMatchResult:
        """Compare two product images using Nemotron vision model."""
        if not image_url1 or not image_url2:
            return ImageMatchResult(is_match=False, confidence=ConfidenceLevel.LOW)
        
        prompt = f"""
Compare these two product images and determine if they show the SAME product.

Consider:
- Same product model/design
- Same color/finish (allow for slight lighting differences)
- Same shape and features
- Different camera angles are acceptable
...
"""
        # For vision models, we need to send base64 encoded images
        # This implementation fetches and encodes the images
        encoded_images = []
        ...
        messages = [{
            ...
            "content": [
                ...
                {"type": "image_url", "image_url": {"url": encoded_images[0]}},
                {"type": "image_url", "image_url": {"url": encoded_images[1]}},
            ]
        }]
        ...
        try:
            ...
            result = ImageMatchResult(raw_response=content)
            json_match = re.search(r'\{[^{}]*\}', content, re.DOTALL)
            if json_match:
                result_data = json.loads(json_match.group())
                result.is_match = result_data.get("is_match", False)
                conf_str = result_data.get("confidence", "LOW")
                result.confidence = ConfidenceLevel[conf_str] if conf_str in ConfidenceLevel.__members__ else ConfidenceLevel.LOW
                result.similarity_score = float(result_data.get("similarity_score", 0.0))
                result.reason = result_data.get("reason", "")
            return result
        except Exception as e:
            print(f"[Vision] Error comparing images: {e}")
            return ImageMatchResult(is_match=False, confidence=ConfidenceLevel.LOW)


# ============================================================================
# Main Validate Class with LLM Integration
# ============================================================================
...
class Validate:
    def __init__(
        ...
        use_llm: bool = True,
        use_vision: bool = False,  # Vision is slower, off by default
        ...
        self._llm_client = NVIDIALLMClient() if use_llm else None
        
        # LLM caches
        self._llm_extraction_cache: Dict[str, LLMExtractionResult] = {}
        self._llm_match_cache: Dict[str, LLMMatchResult] = {}

        # Centralized Scoring Configuration
        self._score_config = copy.deepcopy(DEFAULT_SCORE_CONFIG)
        
        # LLM score bonuses
        self._score_config['llm_high_match'] = 200
        self._score_config['llm_medium_match'] = 100
        self._score_config['llm_low_match'] = 30
        self._score_config['vision_match'] = 150
        ...
        self._allowed_headers = [
            ...
            'reviewed_by_user', 'llm_match_score', 'llm_match_reason', 'vision_match_score'
        ]
    ...
    def _get_llm_extraction(self, url: str) -> Optional[LLMExtractionResult]:
        """Get LLM extraction with caching."""
        if not self._llm_client:
            return None
        if url in self._llm_extraction_cache:
            return self._llm_extraction_cache[url]
        result = self._llm_client.extract_from_url(url)
        self._llm_extraction_cache[url] = result
        return result

    def _get_llm_match(self, system_row: Dict, competitor_row: Dict) -> Optional[LLMMatchResult]:
        """Get LLM match with caching."""
        if not self._llm_client:
            return None
        cache_key = f"{system_row.get('product_id')}_{competitor_row.get('competitor_url', '')}"
        if cache_key in self._llm_match_cache:
            return self._llm_match_cache[cache_key]
        result = self._llm_client.match_products(system_row, competitor_row)

        print("\n******** LLM MATCH DEBUG ********")
        print("System Product:", system_row.get('product_name'))
        print("Competitor Product:", competitor_row.get('Ref Product Name'))
        print("Match:", result.is_match, "| Confidence:", result.confidence)
        print("Score:", result.similarity_score)
        print("Reason:", result.reason)
        print("*********************************\n")

        self._llm_match_cache[cache_key] = result
        return result

    def _enhance_with_llm(self, row: Dict, sys_data: Dict, url: str, 
                          current_score: int, reasons: List[str]) -> Tuple[int, List[str]]:
        """Enhance matching with LLM."""
        if not self._llm_client:
            return current_score, reasons
        
        # Extract from URL if missing MPN/GTIN
        if not sys_data.get('mpn') and not row.get('competitor_sku'):
            extraction = self._get_llm_extraction(url)
            if extraction and extraction.mpn:
                if extraction.mpn not in row.get('competitor_sku', ''):
                    row['competitor_sku'] = extraction.mpn
                    row['competitor_sku1'] = extraction.mpn
                    reasons.append(f"LLM Extracted MPN: {extraction.mpn}")
        
        # LLM matching for ambiguous cases
        competitor_data = {
            'Ref Product Name': row.get('competitor_product_name', ''),
            'Ref MPN': row.get('competitor_sku', ''),
            'Ref GTIN': '',
            'Ref Brand Name': row.get('brand_label', ''),
            'Ref Product URL': url,
        }
        llm_result = self._get_llm_match(sys_data, competitor_data)
        
        if llm_result:
            row['llm_match_score'] = llm_result.similarity_score
            row['llm_match_reason'] = llm_result.reason
            
            if llm_result.is_match:
                if llm_result.confidence == ConfidenceLevel.HIGH:
                    current_score += self._score_config.get('llm_high_match', 200)
                    reasons.append(f"LLM High Match: {llm_result.reason}")
                elif llm_result.confidence == ConfidenceLevel.MEDIUM:
                    current_score += self._score_config.get('llm_medium_match', 100)
                    reasons.append(f"LLM Medium Match: {llm_result.reason}")
                else:
                    current_score += self._score_config.get('llm_low_match', 30)
                    reasons.append(f"LLM Low Match: {llm_result.reason}")
    ...
    def fuzzy_match(self, needle: str, haystack_tokens: list) -> int:
        ...
        fuzzy_threshold = self.get_score_config('fuzzy_match_threshold') or 80
    ...
    def prepare_details_csv(self):
        ...
        with open(comp_file, newline='', encoding='utf-8-sig') as f:
            ...
            for row in reader:
                ...
                new_cols = [
                    ...
                    'llm_match_score', 'llm_match_reason', 'vision_match_score'
                ]
                for col in new_cols:
                    if col not in row:
                        row[col] = ''
                ...
                # LLM Enhancement
                if self._use_llm:
                    score, reasons = self._enhance_with_llm(row, sys_data, url_raw, score, reasons)
                    if row.get('llm_match_score'):
                        llm_usage_count += 1
        ...
        if self._use_llm:
            print(f"LLM Enhanced Rows    : {llm_usage_count}")
        print("--------------------------------------")
    ...
    def _determine_validation_category(self, is_valid, score, min_confidence, valid_url, row):
        ...
        # LLM fallback for manual cases
        if self._use_llm and self._llm_client:
            # Run LLM for ALL non-valid cases (batch-like behavior)
            try:
                sys_data = self._system_data.get(row.get('product_id'), {})
                competitor_data = {
                    'Ref Product Name': row.get('competitor_product_name', ''),
                    'Ref MPN': row.get('competitor_sku', ''),
                    'Ref GTIN': '',
                    'Ref Brand Name': row.get('brand_label', ''),
                    'Ref Product URL': row.get('competitor_url', ''),
                }

                llm_result = self._get_llm_match(sys_data, competitor_data)
                print(f"[LLM CHECK] Product ID: {row.get('product_id')}")

                if llm_result:
                    row['llm_match_score'] = llm_result.similarity_score
                    row['llm_match_reason'] = llm_result.reason

                    if llm_result.is_match and llm_result.confidence in [ConfidenceLevel.HIGH, ConfidenceLevel.MEDIUM]:
                        return 'correct'
                    elif not llm_result.is_match and llm_result.confidence == ConfidenceLevel.HIGH:
                        return 'wrong'
            except Exception as e:
...
# ============================================================================
# Default configurations (kept from original)
# ============================================================================
...
if __name__ == '__main__':
    ...
    obj = Validate(mode, output_type, use_llm=use_llm, max_rows=max_rows)
